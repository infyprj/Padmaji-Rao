...
export class ProductDetailComponent implements OnInit {
  ...
  constructor(
    ...
    private _cartSerive: CartService, // Fixed typo but kept as is to maintain your naming
  ...
  addToCart() {
    ...
    if (!this.userId) {
      ...
      if (this.product) {
        ...
        // Service will automatically update cart count via BehaviorSubject
        this._cartSerive.addProductToCart(cartItem).subscribe(
          responseData => {
            if (responseData) {
              this.showToastMessage("Product Added to Cart", 1);
              // No manual count update needed - service handles it automatically
            } else {
          ...
          responseError => {
            ...
            console.error("Error adding to cart:", this.errorMsg);
          }
        )
      }
    }
  ...
  addToWhishlist() {
    ...
    if (!this.userId) {
      ...
      if (this.product) {
        ...
        // Service will automatically update wishlist count via BehaviorSubject
